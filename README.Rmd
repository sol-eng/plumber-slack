...
API (if it's local or deployed on RStudio Connect). We also provide the API title
and description. Now, we're ready to start defining the filters and endpoints of
the API.
...
verified in this filter to ensure that the request came from Slack.
...
`verify` filter doesn't apply here. Additional work needs to be done in order
...
`verify` filter does not apply to this endpoint.
